...
import { CheckCircle, XCircle, Edit, ArrowLeft, Target, Clock, Users } from 'lucide-react';
...
export function FinalReview() {
  ...
  const averageConfidence = currentPlan.steps.reduce((sum, step) => sum + step.confidence, 0) / currentPlan.steps.length;
  const uniqueTools = [...new Set(currentPlan.steps.map(step => step.toolName))];
  const uniqueAgents = [...new Set(currentPlan.steps.map(step => step.aiAgent))];
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-gray-800/80 backdrop-blur-sm rounded-xl p-8 shadow-lg border border-gray-700">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-300">Review your complete workflow plan before approval</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-gray-700/60 rounded-lg p-4 text-center border border-gray-600">
              ...
              <div className="text-2xl font-bold text-white">{currentPlan.steps.length}</div>
            ...
            <div className="bg-gray-700/60 rounded-lg p-4 text-center border border-gray-600">
              ...
              <div className="text-2xl font-bold text-white">{Math.round(averageConfidence * 100)}%</div>
            ...
            <div className="bg-gray-700/60 rounded-lg p-4 text-center border border-gray-600">
              ...
              <div className="text-2xl font-bold text-white">{uniqueTools.length}</div>
          ...
          <div className="space-y-6 mb-8">
            <div>
              <h3 className="text-lg font-semibold text-white mb-3">Step-by-Step Plan</h3>
              <div className="space-y-3">
                {currentPlan.steps
                  .sort((a, b) => a.order - b.order)
                  .map((step, index) => (
                    <div key={step.id} className="bg-gray-700/60 rounded-lg p-4 border border-gray-600">
            ...
            <div>
              <h3 className="text-lg font-semibold text-white mb-3">Tools & Agents</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gray-700/60 rounded-lg p-4 border border-gray-600">
                  <h4 className="font-medium text-white mb-2">Tools Required</h4>
                  <div className="flex flex-wrap gap-2">
                    {uniqueTools.map(tool => (
                      <span key={tool} className="px-2 py-1 bg-blue-500/20 text-blue-300 text-xs rounded-full border border-blue-500/30">
                        {tool}
                      </span>
                    ))}
                  </div>
                ...
                <div className="bg-gray-700/60 rounded-lg p-4 border border-gray-600">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {uniqueAgents.map(agent => (
                      <span key={agent} className="px-2 py-1 bg-purple-500/20 text-purple-300 text-xs rounded-full border border-purple-500/30">
                        {agent}
                      </span>
                    ))}
                  </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-green-600 to-blue-600 text-white rounded-lg hover:from-green-700 hover:to-blue-700 transition-all duration-200 font-medium shadow-lg shadow-green-500/25"
            >
